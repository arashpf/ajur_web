...
function JoinDeparment(props) {
  ...
  useEffect(() => {
    ...
    if (!token) {
      Cookies.set('destination_before_auth', '/join-department/'+department.id, { expires: 14 });
          ...
          <Grid container spacing={1}>
            ...
            <Grid item xs={12} md={6}>
              <Button
                ...
                onClick={onclickUnregisteredJoinRequest}
  ...
  const onclickUnregisteredJoinRequest = () => {
    
    var token = Cookies.get("id_token");
    if (!token) {
      Cookies.set('destination_before_auth', '/department/'+department.id, { expires: 14 });
      router.push("/panel/auth/login");
    } else {
  ...
  const onclickFinalJoinRequest = () => {
    ...
    if (is_registered) {
      ...
      axios({
        ...
        console.log(
          "response data from department-join-request-for-registered"
        );
        console.log(response.data);
        ...
        if (response.status == 200) {
          if (response.data.agent_status == "exist") {
            ...
            setProblem(
              "درخواست شما با موفقیت ارسال شد ، منتظر تایید مدیریت باشید"
            );
            set_alert_type("success");
            setOpenAlert(true);
            set_loading(false);
          ...
          console.log(
            "something wrong with department-join-request-for-registered "
          );
