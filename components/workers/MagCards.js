    ...
    return (
      <article className="magcard group flex flex-col bg-white rounded-xl overflow-hidden no-underline text-inherit border border-gray-100 shadow-md transition-transform duration-200 ease-out">
        <a data-post-id={post.id} href={post.link} target="_blank" rel="noopener noreferrer" className="block">
          <div className="magcard-image rounded-xl bg-cover bg-center relative h-[180px]" style={{ backgroundImage: post.img ? `url(${post.img})` : 'linear-gradient(#eee,#ddd)' }}>
            <div className="magcard-title absolute right-3 bottom-3 text-white bg-black/45 px-2.5 py-2 rounded-md text-[17px] leading-[1.2] max-w-[90%] text-right group-hover:text-white">{stripHtml(post.title)}</div>
          </div>
        </a>
