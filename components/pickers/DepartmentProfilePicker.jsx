...
import ReactCrop, {
  ...
  const img = {
    ...
    const old_imgage = props.img;

    const defalut_image = "https://api.ajur.app/public/user/images/profile/thumb/upload-image.png";
  ...
  function downscaleImage(value){
    ...
    promise.then((result) => {
      ...
      var converted = dataURLtoFile(result,'proimage.jpg');
     ...
     axios({
        method: 'post',
        url: 'https://api.ajur.app/api/post-department-images',
        data: formData,
        ...
        .then(function (response) {
          ...
          if(response.data.status == 200) {
            ...
            setProblem('عکس دپارتمان با موفقیت بروز شد');
    ...
    const updateAvatar = (dataUrl) => {
      ...
      var resized_images = downscaleImage(dataUrl);
      
    };

    const handleParentLoading = (value) =>{
  ...
  return (
    <>
         ...
         <Tooltip arrow title="انتخاب عکس پروفایل" open={old_imgage == defalut_image ? true : false}>
                  <IconButton onClick={changePicture} sx={{ p: 0 }}>
                    { 
                        image ? 
                        <Avatar style={{background:'#eee'}} alt="pr-pic" src={image}  sx={{ width: size, height: size }}/>
                        :
                        <Avatar style={{background:'#eee'}} alt="pr-pic" src={old_imgage} sx={{ width: size, height: size }}/>
                    }
