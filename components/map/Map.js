...
import CategoryRightSlider from '../categories/CategoryRightSlider';
...
function Test({ location, search }) {
 ...
 function MapNoSsr(props) {
  ...
  //Todo: name and id is hard coded here ! be carefull with that
  const name = 'ویلایی';
  const id = 18;
  ...
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(function(position) {
      ...
      var userInitialLat  = 35.6666;
      var userInitialLong = 52.2222;
  ...
  const renderTopBox = () => {
    return <div className={Styles['top-box-wrapper']}>
      <p>
در حال بروز رسانی هستیم،لطفا بعدا اقدام کنید
      </p>
    </div>
  }
